...
//! Lumina Adaptive Stability Oracle (LASO) — Production
//! Native, regime-aware, stability-impact oracle for a sovereign stablecoin L1.
...
impl PriceFeed {
    ...
    /// Main entry point — real multi-exchange aggregation with P2P staked reporter overlay.
    pub async fn get_latest_report(&mut self, state: &mut GlobalState) -> Result<PriceReport> {
        let mut raw_prices = Vec::new();

        // 1. P2P staked reporters (mainnet path)
    ...
    async fn fetch_kraken(&self, id: &str) -> Result<f64> {
        ...
        let price_str = resp["result"][&pair]["c"][0].as_str().ok_or(anyhow::anyhow!("no price"))?;
        Ok(price_str.parse::<f64>()?)
    }
    ...
    async fn fetch_okx(&self, id: &str) -> Result<f64> {
        let inst_id = format!("{}-USDT", id.to_uppercase());
        let url = format!("https://www.okx.com/api/v5/market/ticker?instId={}", inst_id);
        let resp: Value = self.client.get(&url)
            .timeout(std::time::Duration::from_secs(10))
            .send().await?.json().await?;
        let price_str = resp["data"][0]["last"].as_str().ok_or(anyhow::anyhow!("no price from OKX"))?;
        Ok(price_str.parse::<f64>()?)
    }

    async fn fetch_bitfinex(&self, id: &str) -> Result<f64> {
        let symbol = format!("t{}USD", id.to_uppercase());
        let url = format!("https://api-pub.bitfinex.com/v2/ticker/{}", symbol);
        let resp: Value = self.client.get(&url)
            .timeout(std::time::Duration::from_secs(10))
            .send().await?.json().await?;
        // Bitfinex returns array: [BID, BID_SIZE, ASK, ASK_SIZE, DAILY_CHANGE, ...]
        // Index 6 = LAST_PRICE
        let last_price = resp.as_array()
            .and_then(|arr| arr.get(6))
            .and_then(|v| v.as_f64())
            .ok_or(anyhow::anyhow!("no price from Bitfinex"))?;
        Ok(last_price)
    }

    async fn fetch_bybit(&self, id: &str) -> Result<f64> {
        let symbol = format!("{}USDT", id.to_uppercase());
        let url = format!("https://api.bybit.com/v5/market/tickers?category=spot&symbol={}", symbol);
        let resp: Value = self.client.get(&url)
            .timeout(std::time::Duration::from_secs(10))
            .send().await?.json().await?;
        let price_str = resp["result"]["list"][0]["lastPrice"].as_str()
            .ok_or(anyhow::anyhow!("no price from Bybit"))?;
        Ok(price_str.parse::<f64>()?)
    }

    /// Core LASO aggregation engine
    async fn aggregate_with_laso(&mut self, weighted_prices: &[(f64, f64)]) -> Result<PriceReport> {
        ...
        let filtered: Vec<(f64, f64)> = sorted.iter()
            .filter(|(p, _)| mad < f64::EPSILON || (p - median_price).abs() <= 3.0 * mad)
            .copied()
            .collect();

        // Weighted average
        let total_weight: f64 = filtered.iter().map(|(_, w)| w).sum();
        let final_price = if total_weight > f64::EPSILON {
            filtered.iter().map(|(p, w)| p * w).sum::<f64>() / total_weight
        } else {
            median_price
        };
    ...
    fn is_report_fresh(&self, report: &SignedPriceReport) -> bool {
        ...
        now.saturating_sub(report.timestamp) < 300
    }
